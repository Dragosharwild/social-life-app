...
class FeedScreen(ttk.Frame):
    """Scrollable feed + right-side nav (ported from legacy feed.py)."""
    ...
    def __init__(self, parent, show_callback):
        ...
        ttk.Button(header, text="Back to Interests", command=lambda: self.show("interests")).grid(
        ...
        # Right sidebar nav
        sidebar = Sidebar(self, self.show)
    ...
    def render_posts(self, posts):
        ...
        for i, p in enumerate(posts):
            ...
            ttk.Label(card, text=p["bubble"], font=("Segoe UI", 11), foreground="#666").grid(
