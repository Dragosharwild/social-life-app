...
def _centered_card(parent):
    """Create a centered frame using a 3×3 grid."""
    for r in range(3):
...
class InterestsScreen(ttk.Frame):
    def __init__(self, parent, show_callback):
        ...
        ttk.Label(card, text="Pick your interests", font=("Segoe UI", 14, "bold")).grid(row = 0, column = 0, columnspan = 3, pady = (0, 12))
        ...
        ttk.Button(card, text = "Back", command = lambda: self.show("login")).grid(row = 3, column = 0, sticky = "w", pady = (12, 0))
        ttk.Button(card, text = "Continue", command = self._continue).grid(row = 3, column = 2, sticky = "e", pady = (12, 0))
    ...
    def refresh(self):
        ...
        for idx, item in enumerate(items):
            ...
            ttk.Button(frame, text = btn_text,
                       command=lambda it = item: self._toggle_follow(it["id"])).pack(pady = (5,0))
